...
// Ambil bookmark dari localStorage dan dari API saat mounted
const fetchBookmarkedStories = async () => {
  ...
  try {
    ...
    // Simpan data bookmark ke state (menggunakan Set)
...
// Fungsi toggle bookmark yang sudah diperbaiki agar reaktivitas bekerja
const toggleBookmark = async (storyId) => {
  ...
  try {
    ...
    // Buat Set baru untuk memastikan reaktivitas terpicu
    if (bookmarkedStories.value.has(storyId)) {
      const newSet = new Set(bookmarkedStories.value);
      newSet.delete(storyId);
      bookmarkedStories.value = newSet;
    } else {
      const newSet = new Set(bookmarkedStories.value);
      newSet.add(storyId);
      bookmarkedStories.value = newSet;
...
// Muat bookmark saat komponen di-mount
onMounted(() => {
    ...
    <nav class="bg-[#F0F5ED] py-6">
      <div class="container mx-auto text-sm text-[#466543] pl-12">
        <NuxtLink to="/" class="relative inline-block group">
          Home
          <span class="absolute left-1/2 bottom-0 h-[2px] w-0 bg-[#466543] transition-all duration-300 ease-out group-hover:left-0 group-hover:w-full"></span>
        </NuxtLink>
      ...
      <div class="relative w-full rounded-lg overflow-hidden">
        ...
        <div v-if="isModalOpen"
          ...
          <div class="relative bg-white p-10 rounded-lg shadow-lg max-w-3xl w-full max-w-[50%]">
            <button class="absolute top-4 right-2 bg-[#466543] text-white rounded-full w-8 h-8 flex items-center justify-center hover:bg-[#3B4F3A]"
      ...
      <a href="#" target="_blank" class="social-icon">
        <div class="icon-container-facebook">
          <svg xmlns="http://www.w3.org/2000/svg" width="96" height="70" viewBox="0 0 70 70" fill="white"
            ...
            <path d="M39 21h-6c-2 0-4 2-4 4v5h-5v8h5v19h9V38h6l1-8h-7v-4c0-1 1-2 2-2h5v-8h-6c-6 0-10 4-10 10v4h-5v8h5v19h9V38h6l1-8h-7v-5c0-1 1-2 2-2h5v-8z" fill="white" />
          </svg>
        </div>
      </a>
